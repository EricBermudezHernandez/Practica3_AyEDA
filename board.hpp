...
void Board<Base>::Start() {
  BigInt<Base> operando_1, operando_2,
      resultado;  // Se definen los operandos para las operaciones y el
                  // resultado
  ...
  while (
      std::getline(fichero_, linea)) {  // Se itera sobre las líneas del fichero
    ...
    while (linea_stream >> palabra) {  // Se itera sobre las palabras restantes de la línea
      if (variables_.find(palabra) !=
          variables_.end()) {  // Si la palabra es una variable ya definida, se
